...
class RequestHandler():
    ...
    def ListOfSuggestedFees(self):
        try:
            exeQuery=RssFeeds.objects.filter(rss_category__rss_is_suggested=True,rss_category__rss_is_active=True).exclude(rss_description__isnull=True).order_by('-rss_publish_date','-rss_views_no')[:5]
            return exeQuery
        except Exception as e:
